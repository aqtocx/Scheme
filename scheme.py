...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = expr.second.map(lambda x: scheme_eval(x, env))
...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions.first, env, True)    #the last subexpression is always in tail context
    # END PROBLEM 8
...
def do_begin_form(expressions, env):
    ...
    return eval_all(expressions, env)   #eval_all is optimized
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    return LambdaProcedure(formals, expressions.second, env)    #body = expressions.second
    # END PROBLEM 9
...
#old recursive version does not support tail optimization
    result = True
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                return test
            return eval_all(clause.second, env)     #eval_all is optimized
            # END PROBLEM 14
        expressions = expressions.second
...
def do_let_form(expressions, env):
    ...
    return eval_all(expressions.second, let_env)    #eval_all is optimized
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        check_form(bindings.first, 2, 2)    #must have 1 formal and 1 val
        check_formals(Pair(bindings.first.first, nil))  #bindings.first.first is formal, and bindings.first.second.first is the expr for val
